...
namespace ChanSort.Loader.SamsungJ
{
  class DbSerializer : SerializerBase
  {
    ...
    private void ReadTransponders(SQLiteCommand cmd)
    {
      ...
      using (var r = cmd.ExecuteReader())
      {
        while (r.Read())
        {
          ...
          tp.Number = id;
          // 171027 - ohuseyinoglu: Samsung Q series shows the polarity (with "Right", "Left" indicators as well) as: "H/L", "V/R"
          // But since the underlying type is a char, we will leave the good-old polarity indicators!
    ...
    private void ReadChannelDatabase(SQLiteConnection conn, string dbPath, bool digital)
    {
      ...
      using (var cmd = conn.CreateCommand())
      {
        // 171027 - ohuseyinoglu: No need to call RepairCorruptedDatabaseImage again here since it gets called in our caller
